...
| GitHub secret scanning / Dependabot alerts enabled                | **Needs a live check** — a repository _setting_ (Settings → Code security), not a file; `dependabot.yml` only covers version-update PRs, not this                                                                                                                                                                                                                           |
...
| Answer editor offline/reconnect behavior                    | **Verified in code** for the retry/local-recovery logic itself (`AnswerEditor.tsx` + its tests); the _felt_ experience of an actual dropped connection still deserves a live check |
