...
-- Fetching bob's own token here stands in for "the token embedded in
-- the unsubscribe link he already has from an email" — it's a test
-- fixture lookup, not something the app does as an anonymous reader,
-- so it runs as service_role (0004's migration only grants
-- select/update on email_preferences to `authenticated`, not `anon`).
select tests.become_service_role();
...
-- Back to service_role to verify the row directly, same reasoning as
-- the token fetch above.
select tests.become_service_role();
select is(
